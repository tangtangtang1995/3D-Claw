...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.1.1/Polygon_mesh_processing/include/CGAL/Polygon_mesh_processing/approximated_centroidal_Voronoi_diagram_remeshing.h $
// $Id: include/CGAL/Polygon_mesh_processing/approximated_centroidal_Voronoi_diagram_remeshing.h 08b27d3db14 $
...
// === Visitor for process visualization (Easy3D extension) ===
// Default no-op visitor. Override specific callbacks to intercept algorithm
// events. Pattern matches Efficient_RANSAC_default_visitor and the
// Region_growing_default_visitor. Used with the existing CGAL visitor_t
// named parameter tag (parameters_interface.h:18).

struct ACVD_default_visitor
{
  template <typename Mesh>
  void on_acvd_begin(const Mesh&, int /*target_clusters*/) {}

  template <typename Mesh>
  void on_preprocessing_begin(const Mesh&) {}

  template <typename Mesh>
  void on_preprocessing_end(const Mesh&) {}

  template <typename VertexDescriptor, typename Point>
  void on_seed_created(int /*cluster_id*/, VertexDescriptor, const Point&) {}

  void on_clustering_loop_begin(int /*loop*/, bool /*qem_energy*/) {}

  template <typename Mesh, typename VertexClusterMap,
            typename VertexDescriptor, typename Point>
  void on_vertex_assigned(const Mesh&,
                          const VertexClusterMap&,
                          VertexDescriptor,
                          int /*from_cluster*/,
                          int /*to_cluster*/,
                          const Point&) {}
  ...
  template <typename Mesh, typename VertexClusterMap>
  void on_iteration_end(const Mesh& /*tmesh*/,
                        const VertexClusterMap& /*cluster_pmap*/,
                        int /*loop*/,
                        int /*iteration*/,
                        int /*modifications*/,
                        int /*assigned_vertices*/,
                        int /*nb_clusters*/,
                        bool /*qem_energy*/) {}

  void on_disconnected_clusters_fixed(int /*loop*/,
                                      int /*disconnected_clusters*/) {}

  void on_output_soup_built(std::size_t /*points*/,
                            std::size_t /*polygons*/,
                            std::size_t /*non_manifold_clusters*/) {}

  void on_non_manifold_repair_begin(std::size_t /*bad_clusters*/) {}
  void on_non_manifold_repair_end(int /*new_cluster_count*/) {}

  template <typename Mesh>
  void on_acvd_end(const Mesh&, bool /*success*/) {}

  template <typename Mesh>
  bool go_further(const Mesh&) { return true; }
};
namespace internal {
          ...
          const NamedParameters& np = parameters::default_values())
{
  ...
  using Visitor = typename internal_np::Lookup_named_param_def<
      internal_np::visitor_t, NamedParameters, ACVD_default_visitor>::type;
  Visitor visitor = choose_parameter(
      get_parameter(np, internal_np::visitor), ACVD_default_visitor());
  visitor.on_acvd_begin(tmesh, nb_clusters);
  ...
  visitor.on_preprocessing_end(tmesh);
  ...
  if constexpr (is_rand_access)
  {
    for (int ci = 0; ci < nb_clusters; ++ci)
    {
      ...
      visitor.on_seed_created(ci, vd, get(vpm, vd));
    }
  }
  else
  {
  ...
  do
  {
    visitor.on_clustering_loop_begin(nb_loops, qem_energy_minimization);
    do
    {
      if (!visitor.go_further(tmesh))
        break;
      // reset cluster data
      for (int ci=0; ci<nb_clusters; ++ci)
      ...
      do
      {
        ...
        while (!clusters_edges_active.empty())
        {
          ...
          if (c1 == -1)
          {
            ...
            visitor.on_vertex_assigned(tmesh, vertex_cluster_pmap,
                                       v1, -1, c2, get(vpm, v1));
      ...
      visitor.on_iteration_end(tmesh, vertex_cluster_pmap,
                               nb_loops, nb_iterations,
                               nb_modifications,
                               (int)num_vertices(tmesh), nb_clusters,
                               qem_energy_minimization);
      ...
      visitor.on_disconnected_clusters_fixed(nb_loops, nb_disconnected);
#ifdef CGAL_DEBUG_ACVD
      std::cout << "# nb_disconnected: " << nb_disconnected << "\n";
#endif
      ++nb_loops;

    } while ((nb_disconnected > 0 || nb_loops < 3) && visitor.go_further(tmesh));
    ...
    visitor.on_output_soup_built(points.size(), polygons.size(),
                                 /*non_manifold yet TBD */ 0);
    ...
    if (nm_clusters.empty())
    {
      ...
      visitor.on_acvd_end(tmesh, true);
