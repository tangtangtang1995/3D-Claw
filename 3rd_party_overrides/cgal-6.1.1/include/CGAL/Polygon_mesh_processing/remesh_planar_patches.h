...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.1.1/Polygon_mesh_processing/include/CGAL/Polygon_mesh_processing/remesh_planar_patches.h $
// $Id: include/CGAL/Polygon_mesh_processing/remesh_planar_patches.h 08b27d3db14 $
...
struct Default_visitor
{
  ...
  // === Easy3D optional process callbacks ===
  // Subclass and override to capture patch-by-patch progress. The base
  // class supplies no-op defaults so any caller passing Default_visitor
  // (or a Visitor that derives from it) compiles unchanged.
  void on_patch_detection_done(std::size_t /*nb_patches*/,
                               std::size_t /*nb_corners*/,
                               std::size_t /*nb_constrained_edges*/) const {}
  void on_patch_begin(std::size_t /*patch_id*/,
                      std::size_t /*input_face_count*/,
                      std::size_t /*boundary_edge_count*/) const {}
  void on_patch_triangulated(std::size_t /*patch_id*/,
                             std::size_t /*output_face_count*/) const {}
  void on_patch_fallback(std::size_t /*patch_id*/,
                         int /*reason_code*/) const {}
  void on_patch_end(std::size_t /*patch_id*/) const {}
  void on_polygon_soup_ready(std::size_t /*corners_count*/,
                             std::size_t /*faces_count*/) const {}
  void on_output_mesh_built(std::size_t /*output_vertices*/,
                            std::size_t /*output_faces*/) const {}
  // Cooperative cancel hook — return false to bail out between patches.
                   ...
                   Visitor& visitor)
{
  ...
  // Easy3D: count constrained edges once for the detection-done
  // callback, then signal that the algorithm now knows how many patches /
  // corners / borders it is about to remesh.
  {
    std::size_t nb_ce = 0;
    for (auto e : edges(tm))
      if (get(edge_is_constrained, e)) ++nb_ce;
    visitor.on_patch_detection_done(nb_corners_and_nb_cc.second,
                                    nb_corners_and_nb_cc.first,
                                    nb_ce);
  }
  ...
  do
  {
    ...
    // Easy3D: precompute face count per cc once per retry so the
    // on_patch_begin callback can report it without a per-patch O(F) scan.
    std::vector<std::size_t> _ppr_face_count_per_cc(
        nb_corners_and_nb_cc.second, 0);
    for (face_descriptor _ppr_f : faces(tm))
    {
      typename boost::property_traits<FaceCCIdMap>::value_type _ppr_cc =
          get(face_cc_ids, _ppr_f);
      if (_ppr_cc < (typename boost::property_traits<FaceCCIdMap>::value_type)(
                        _ppr_face_count_per_cc.size()))
        ++_ppr_face_count_per_cc[(std::size_t)_ppr_cc];
    }
    bool _ppr_break_for_cancel = false;
                         ...
                         cc_id = cc_to_handle.find_next(cc_id))
    {
      ...
      // Process hooks: cooperative cancel + patch-begin.
      if (!visitor.go_further()) { _ppr_break_for_cancel = true; break; }
      visitor.on_patch_begin(cc_id,
                             _ppr_face_count_per_cc[cc_id],
                             csts.size());
      ...
      if (csts.size()==3)
      {
        ...
        visitor.on_patch_triangulated(cc_id, cc_faces.size());
        visitor.on_patch_end(cc_id);
        continue;
      }
      else
      {
        ...
        else
        {
          ...
          cc_to_handle.set(cc_id, 0);
          visitor.on_patch_fallback(cc_id, 0);
          visitor.on_patch_end(cc_id);
        }
      }
    }
    if (_ppr_break_for_cancel) break;
  }
                   ...
                   std::vector<typename Kernel::Vector_3>& face_normals)
{
  ...
  visitor.on_polygon_soup_ready(corners.size(), faces.size());
  visitor(pm_out);
  ...
  visitor.on_output_mesh_built(num_vertices(pm_out), num_faces(pm_out));
...
//  bool no_remeshing_issue = true;
  do{
    ...
    for(std::size_t mesh_id=0; mesh_id<nb_meshes; ++mesh_id)
    {
      ...
      // This multi-mesh helper has no visitor parameter — fabricate
      // a local Default_visitor so the augmented inner decimate_impl
      // signature still resolves.
      Default_visitor<Triangle_mesh> _ppr_multi_default_visitor;
                           ...
                           const NamedParametersOut& np_out = parameters::default_values())
{
  ...
  auto visitor = choose_parameter(get_parameter(np_out, internal_np::visitor),
                                    Planar_segmentation::Default_visitor<PolygonMeshOut>());
