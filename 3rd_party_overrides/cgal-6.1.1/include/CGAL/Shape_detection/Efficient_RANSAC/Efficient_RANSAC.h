...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.1.1/Shape_detection/include/CGAL/Shape_detection/Efficient_RANSAC/Efficient_RANSAC.h $
// $Id: include/CGAL/Shape_detection/Efficient_RANSAC/Efficient_RANSAC.h 08b27d3db14 $
...
// Default visitor for Efficient_RANSAC — all callbacks are no-ops.
// Derive from this to receive algorithm events. Template methods accept
// the Efficient_RANSAC instance (read-only) as first parameter.
struct Efficient_RANSAC_default_visitor {
    Efficient_RANSAC_default_visitor() {}
    ...
    void on_candidate(const RANSAC&, const Shape* /*shape*/, double /*score*/) {}
    template <typename RANSAC, typename Shape>
    void on_inliers(const RANSAC&, const Shape* /*shape*/, int /*count*/) {}
    template <typename RANSAC, typename Shape>
    void on_shape_accepted(const RANSAC&, const Shape* /*shape*/, int /*id*/) {}
    template <typename RANSAC, typename Shape>
    void on_shape_rejected(const RANSAC&, const Shape* /*shape*/) {}
    template <typename RANSAC>
...
public:
  ...
  /// @{
  /// \cond SKIP_IN_MANUAL
  typedef Traits Traits_type; ///< Traits type (for visitor access).
...
public:
  ...
  */
  // Original signature (backwards-compatible, no visitor)
  bool detect(const Parameters &options = Parameters(),
              const std::function<bool(double)> &callback
              = std::function<bool(double)>()) {
    Efficient_RANSAC_default_visitor noop;
    return detect_with_visitor(options, noop, callback);
  }

  // Visitor-enabled signature — injects algorithm process events.
  template <typename Visitor>
  bool detect_with_visitor(const Parameters &options,
                           Visitor& visitor,
                           const std::function<bool(double)> &callback
                           = std::function<bool(double)>()) {
    ...
    do { // main loop
      ...
      if (keep_searching)
        do {
            ...
            for (std::size_t nb = 0; nb < search_number; ++ nb)
            {
              ...
              visitor.on_sampling(*this, indices);
              ...
              for(typename std::vector<Shape *(*)()>::iterator it =
                ...
                if (p->is_valid()) {
                  ...
                  if(p->max_bound() >= m_options.min_points && p->score() > 0) {
                    ...
                    visitor.on_candidate(*this, p, p->score());
      ...
      visitor.on_inliers(*this, best_candidate,
                         (int)best_candidate->indices_of_assigned_points().size());
      ...
      if (best_candidate->indices_of_assigned_points().size() <
        ...
        visitor.on_shape_accepted(*this, best_candidate,
        ...
        visitor.on_peel(*this, (int)(m_num_available_points - num_invalid));
