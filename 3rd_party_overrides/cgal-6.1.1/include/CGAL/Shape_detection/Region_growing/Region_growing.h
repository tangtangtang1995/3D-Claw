...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.1.1/Shape_detection/include/CGAL/Shape_detection/Region_growing/Region_growing.h $
// $Id: include/CGAL/Shape_detection/Region_growing/Region_growing.h 08b27d3db14 $
...
namespace Shape_detection {
  ...
  // Default no-op visitor. Override specific callbacks to intercept
  // algorithm events. Pattern matches Efficient_RANSAC_default_visitor
  // in Efficient_RANSAC.h.

  struct Region_growing_default_visitor {
      template <typename Item>
      void on_seed_selected(const Item&) {}
      template <typename Item>
      void on_frontier_item(const Item&) {}
      template <typename Item>
      void on_neighbor_accepted(const Item&, const Item&) {}
      template <typename Item>
      void on_neighbor_rejected(const Item&, const Item&) {}
      template <typename Region>
      void on_primitive_refit(const Region&) {}
      void on_fit_check(bool) {}
      template <typename Region>
      void on_region_accepted(int /*region_id*/, const Region&) {}
...
namespace internal {
  ...
  */
  template<
    ...
    typename Visitor = Region_growing_default_visitor>
    ...
    PrimitiveAndRegionOutputIterator detect(PrimitiveAndRegionOutputIterator region_out = PrimitiveAndRegionOutputIterator()) {
      ...
      for (auto it = m_seed_range.begin(); it != m_seed_range.end(); it++) {
        ...
        if (!get(m_visited, seed)) {
          m_visitor.on_seed_selected(seed);
          const bool is_success = propagate(seed, region);

          // Check global conditions.
          if (!is_success || !m_region_type.is_valid_region(region)) {
            m_visitor.on_region_rejected();
            revert(region);
          }
          else {
            m_visitor.on_region_accepted(m_nb_regions, region);
        ...
        m_visitor.on_progress(
          (int)std::distance(m_seed_range.begin(), it) + 1,
          (int)m_seed_range.size(), (int)m_nb_regions);
        if (m_visitor.should_stop())
          break;
      }
      ...
      while (grown) {
        ...
        while (
          ...
          while (!running_queue[depth_index].empty()) {
            ...
            m_visitor.on_frontier_item(item);
            ...
            for (Item neighbor : neighbors) {
              ...
              if (!get(m_visited, neighbor)) {
                if (m_region_type.is_part_of_region(neighbor, region)) {
                  m_visitor.on_neighbor_accepted(item, neighbor);
                  ...
                  m_visitor.on_neighbor_rejected(item, neighbor);
        ...
        if (grown) {
          ...
          m_visitor.on_primitive_refit(region);
