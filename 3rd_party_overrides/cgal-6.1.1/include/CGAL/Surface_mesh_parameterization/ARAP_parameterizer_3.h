...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.1.1/Surface_mesh_parameterization/include/CGAL/Surface_mesh_parameterization/ARAP_parameterizer_3.h $
// $Id: include/CGAL/Surface_mesh_parameterization/ARAP_parameterizer_3.h 08b27d3db14 $
// SPDX-License-Identifier: GPL-3.0-or-later OR LicenseRef-Commercial
//
// Author(s)     : Mael Rouxel-Labbe
...
// Default no-op visitor for ARAP iteration callbacks (Easy3D hook).
struct ARAP_default_visitor {
    template <typename TriangleMesh, typename VertexUVmap, typename VertexIndexMap>
...
template < class TriangleMesh_,
           ...
           class Visitor_ = ARAP_default_visitor>
...
private:
  ...
  // Easy3D: iteration visitor (e.g. snapshot collection).
  Visitor_ m_visitor;

// Public mutator for the visitor
public:
  void set_visitor(const Visitor_& v) { m_visitor = v; }
...
public:
                          ...
                          VertexParameterizedMap vpmap)
  {
    ...
    // Easy3D visitor: iteration 0 = initial parameterization.
    m_visitor.after_iteration(0, CGAL::to_double(energy_this),
                               mesh, uvmap, vimap);
    ...
    for(;;)
    {
      ...
      // Easy3D visitor: after global phase (UV updated). Report the energy
      // for the updated UV map, not the previous iteration's value.
      m_visitor.after_iteration(ite,
          CGAL::to_double(energy_this), mesh, uvmap, vimap);

      // energy based termination
      if(m_tolerance > 0. && ite <= m_iterations) {
