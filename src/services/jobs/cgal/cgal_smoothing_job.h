// SPDX-License-Identifier: GPL-3.0-only
// Copyright (C) 2026 3D Claw contributors

#ifndef CLAW3D_SERVICES_CGAL_SMOOTHING_JOB_H
#define CLAW3D_SERVICES_CGAL_SMOOTHING_JOB_H

/// Service facade for launching and polling CGAL smoothing jobs.

#include "common/model_handle.h"
#include "common/smoothing_contract.h"

#include <atomic>
#include <functional>
#include <memory>
#include <string>
#include <vector>

namespace easy3d {
class SurfaceMesh;
}

class AlgorithmController;

namespace claw3d::services {

struct CgalSmoothingJobStart;

/// UI-facing handle for an asynchronous backend job.
/// Keeps concrete runner types out of the app layer.
class CgalSmoothingJobHandle {
public:
    CgalSmoothingJobHandle() = default;

    bool valid() const;
    explicit operator bool() const { return valid(); }
    void reset();

    void cancel() const;
    bool is_cancelled() const;
    bool is_done() const;
    bool has_error() const;
    std::string last_error() const;
    bool cancelled_or_failed() const;
    bool done_and_ready(const std::atomic<bool>& ready) const;
    void copy_error_if_any(std::string& error) const;
    bool poll_snapshot(int& last_generation, SMOOTH_Snapshot& out) const;
    SMOOTH_ResultStats result_stats() const;

private:
    friend CgalSmoothingJobHandle start_cgal_smoothing_job(
        AlgorithmController& controller,
        const CgalSmoothingJobStart& request);

    struct Impl;

    explicit CgalSmoothingJobHandle(std::shared_ptr<Impl> impl);

    std::shared_ptr<Impl> impl_;
};

/// Start request captured before the backend worker is launched.
struct CgalSmoothingJobStart {
    easy3d::SurfaceMesh* source_mesh = nullptr;
    ModelHandle source_handle;
    SMOOTH_Config config;
    std::string source_name;
    std::atomic<bool>* final_result_ready = nullptr;
    std::function<void()> wake_ui;
};

CgalSmoothingJobHandle start_cgal_smoothing_job(
    AlgorithmController& controller,
    const CgalSmoothingJobStart& request);

} // namespace claw3d::services

#endif // CLAW3D_SERVICES_CGAL_SMOOTHING_JOB_H
