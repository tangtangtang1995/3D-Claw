...
namespace claw3d::algo {
    ...
    using easy3d::cross;
    using easy3d::dot;
    using easy3d::length2;
    using easy3d::norm;
    using easy3d::normalize;
    using easy3d::vec2;
    using easy3d::vec3;

    ObservableSurfaceMeshGeodesic::ObservableSurfaceMeshGeodesic(easy3d::SurfaceMesh *mesh, bool use_virtual_edges)
    ...
    ObservableSurfaceMeshGeodesic::~ObservableSurfaceMeshGeodesic() = default;

    //-----------------------------------------------------------------------------

    void ObservableSurfaceMeshGeodesic::find_virtual_edges() {
        easy3d::SurfaceMesh::Halfedge hh, hhh;
        easy3d::SurfaceMesh::Vertex vh0, vh1, vhn, start_vh0, start_vh1;
        ...
        const float max_angle = static_cast<float>(90.0 / 180.0 * M_PI);
                        ...
                        while (((vh0 == start_vh0) || (vh1 == start_vh1)) &&
                            ...
                            if ((fabs(vn[1]) / fabs(vn[0])) < tan_beta) {
                                virtual_edges_.insert(
                                        std::pair<easy3d::SurfaceMesh::Halfedge, VirtualEdge>(
    ...
    unsigned int ObservableSurfaceMeshGeodesic::compute(const std::vector<easy3d::SurfaceMesh::Vertex> &seed,
    ...
    unsigned int ObservableSurfaceMeshGeodesic::init_front(const std::vector<easy3d::SurfaceMesh::Vertex> &seed,
                                                 std::vector<easy3d::SurfaceMesh::Vertex> *neighbors) {
        unsigned int num(0);
    ...
    unsigned int ObservableSurfaceMeshGeodesic::propagate_front(float max_dist,
        ...
        while (!front_->empty()) {
            ...
            // Live-visualization observer hook. Default nullptr,
            // zero overhead for existing callers.
            if (observer_) {
                observer_->on_vertex_settled(num, v, distance_[v],
                                             front_->size());
                if (!observer_->go_further())
                    break;
            }
    ...
    void ObservableSurfaceMeshGeodesic::heap_vertex(easy3d::SurfaceMesh::Vertex v) {
        assert(!processed_[v]);

        easy3d::SurfaceMesh::Vertex v0, v1, vv;
    ...
    float
    ObservableSurfaceMeshGeodesic::distance(easy3d::SurfaceMesh::Vertex v0, easy3d::SurfaceMesh::Vertex v1, easy3d::SurfaceMesh::Vertex v2, float r0,
                                  float r1) {
        vec3 A, B, C;
        double TA, TB;
        double a, b;
        ...
        if (c < 0.0)
            return static_cast<float>(dykstra);
    ...
    void ObservableSurfaceMeshGeodesic::distance_to_texture_coordinates() {
